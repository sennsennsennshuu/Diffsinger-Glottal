...
def train_model(
    ...
    # ── Auto data_hours ──
    _info_path = binary_data_dir / "train.info.npz"
    if _info_path.exists():
        import numpy as np
        _info = dict(np.load(_info_path, allow_pickle=True))
        _lengths = _info.get("lengths")
        if _lengths is not None:
            _hop = getattr(config.binarizer.features, 'hop_size', 512)
            _sr = getattr(config.binarizer.features, 'audio_sample_rate', 44100)
            _frames_per_sec = _sr / _hop
            _total_frames = int(_lengths.sum())
            _hours = _total_frames / _frames_per_sec / 3600
            _max_steps = config.training.trainer.max_steps
            _step = max(10000, _max_steps // 8) if _hours < 20 else _max_steps // 8
            rank_zero_info(
                f"| data_stats: {_total_frames:,} frames × {_hop}hop@{_sr}Hz "
                f"= {_hours:.1f}h ({len(_lengths)} clips, incl. augmentation) → "
                f"recommend lr_scheduler.kwargs.step_size={_step}, "
                f"p1_pct=0.25, p2_pct=0.50, p3_pct=0.75"
            )
