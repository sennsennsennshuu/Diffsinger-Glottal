...
def find_exp(exp):
    # v3-sfcb: checkpoints are in experiments/, not checkpoints/
    exp_dir = root_dir / 'experiments' / exp
    if not exp_dir.exists():
        for subdir in (root_dir / 'experiments').iterdir():
            ...
            raise click.BadParameter(
                f'There are no matching experiment starting with \'{exp}\' in \'experiments\' folder. '
