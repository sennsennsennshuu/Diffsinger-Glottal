...
class DiffSingerAcousticInfer(BaseSVSInfer):
    def __init__(self, device=None, load_model=True, load_vocoder=True, ckpt_steps=None):
        ...
        if load_model:
            ...
            if hasattr(self.model, 'glottal_predictor') and self.model.glottal_predictor is not None:
                self.model.glottal_predictor.eval()
    ...
    def preprocess_input(self, param, idx=0):
        ...
        # SFC Breathness: glottal blend parameter (frame-level, -1 to +1)
        if param.get('glottal_blend') is not None:
            glottal_blend = resample_align_curve(
                np.array(param['glottal_blend'].split(), np.float32),
                original_timestep=float(param['glottal_blend_timestep']),
                target_timestep=self.timestep,
                align_length=length
            )
            batch['glottal_blend'] = torch.clip(
                torch.from_numpy(glottal_blend.astype(np.float32)).to(self.device)[None],
                min=-1., max=1.
            )
            summary['glottal_blend'] = 'manual'
        else:
    ...
    def forward_model(self, sample):
        ...
        mel_pred: ShallowDiffusionOutput = self.model(
            ...
            glottal_blend=sample.get('glottal_blend'),
            infer=True
        )
