...
class BaseExporter:
    ...
    def export_dictionaries(self, path: Path):
        """Copy dictionary files to the export directory.

        Sources are resolved from hparams['dictionaries'] (e.g.
        {'zh': 'dictionaries/opencpop-extension.txt'}) relative to
        the project root, NOT from work_dir.
        """
        dicts = hparams.get('dictionaries')
        project_root = pathlib.Path(__file__).resolve().parents[1]
        if dicts is not None:
            for lang, dict_path in dicts.items():
                src = pathlib.Path(dict_path)
                if not src.is_absolute():
                    src = project_root / src
                dst = path / f'dictionary-{lang}.txt'
                if src.exists():
                    shutil.copy(src, dst)
                    print(f'| export dictionary => {dst}')
                else:
                    print(f'| WARNING: dictionary not found at {src}, skipping')
        else:
            # Fallback: try project-root dictionaries/dictionary.txt
            fallback = project_root / 'dictionaries' / 'dictionary.txt'
            if fallback.exists():
                shutil.copy(fallback, path / 'dictionary.txt')
                print(f'| export dictionary => {path / "dictionary.txt"}')
            else:
                print('| WARNING: no dictionary found to export')
