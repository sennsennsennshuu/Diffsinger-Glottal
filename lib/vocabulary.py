...
def load_phoneme_dictionary(dictionaries: dict[str, pathlib.Path]) -> PhonemeDictionary:
    """Load phoneme dictionary from paths — convenience wrapper for ONNX export.

    The acoustic_exporter.py references this function to build the dictionary
    before exporting the ONNX model.
    """
    return PhonemeDictionary(dictionaries=dictionaries)
