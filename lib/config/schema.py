...
class BinarizerFeaturesConfig(ConfigBaseModel):
    ...
    aperiodic_mel: CurveParameterConfig = Field(
        CurveParameterConfig(enabled=False),
        json_schema_extra={"scope": ConfigurationScope.ACOUSTIC}
    )
...
# =============================================================================
# SFC Breathness Config
# =============================================================================

class SweetBreathyGlottalConfig(ConfigBaseModel):
    """LF glottal conditioner and DDSP hybrid module configuration.
    Default enabled — set enabled=false to disable all SFC Breathness enhancements."""
    enabled: bool = Field(True)
    use_lf_glottal: bool = Field(True)
    lf_param_dim: int = Field(5, ge=3)
    glottal_hidden_dim: int = Field(128, gt=0)


class SweetBreathyLossConfig(ConfigBaseModel):
    """SFC Breathness loss weight configuration.
    Default enabled — set enabled=false to disable breathness-specific losses."""
    enabled: bool = Field(True)
    mel_l1_weight: float = Field(1.0, gt=0)
    high_freq_weight: float = Field(0.5, gt=0)
    spectral_tilt_weight: float = Field(0.3, gt=0)
    breathiness_weight: float = Field(1.5, ge=0)
    voicing_weight: float = Field(1.2, ge=0)
    h1h2_weight: float = Field(0.8, gt=0)
    hnr_weight: float = Field(0.5, gt=0)
    air_energy_weight: float = Field(0.6, gt=0)
    noise_modulation_weight: float = Field(0.3, gt=0)
    fant_consistency_weight: float = Field(0.4, gt=0)
    warmup_start_step: int = Field(50000, ge=0)
    warmup_full_step: int = Field(64000, ge=0)
    # Per-loss warmup schedule (start_step, full_step). Linear ramp between.
    # Missing losses default to no warmup (always full).
    loss_warmup: Dict[str, List[int]] = Field(default_factory=dict)
    # EMA decay for per-loss magnitude normalization (0=instant, 1=never update)
    adaloss_ema_decay: float = Field(0.99, ge=0, le=1)
...
class ModelConfig(ConfigBaseModel):
    ...
    sweet_breathy: SweetBreathyGlottalConfig | None = Field(
        SweetBreathyGlottalConfig(),
        json_schema_extra={"scope": ConfigurationScope.ACOUSTIC | ConfigurationScope.VARIANCE}
    )
...
class LossConfig(ConfigBaseModel):
    ...
    sweet_breathy: SweetBreathyLossConfig | None = Field(
        SweetBreathyLossConfig(),
        json_schema_extra={"scope": ConfigurationScope.ACOUSTIC | ConfigurationScope.VARIANCE}
    )
...
class OptimizerConfig(ConfigBaseModel):
    ...
    kwargs: dict[str, Any] = Field(...)
    aux_lr_scale: float = Field(1.0, gt=0, json_schema_extra={"scope": ConfigurationScope.ACOUSTIC})
    ddsp_lr_scale: float = Field(1.0, gt=0, json_schema_extra={"scope": ConfigurationScope.ACOUSTIC})
    cvae_lr_scale: float = Field(1.0, gt=0, json_schema_extra={"scope": ConfigurationScope.ACOUSTIC})
    glottal_lr_scale: float = Field(1.0, gt=0, json_schema_extra={"scope": ConfigurationScope.ACOUSTIC})
...
class VocoderConfig(ConfigBaseModel):
    vocoder_type: Literal["nsf-hifigan", "pc-nsf-hifigan"] = Field("nsf-hifigan", json_schema_extra={
        "scope": ConfigurationScope.ACOUSTIC
    })
