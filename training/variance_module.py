...
class VarianceLightningModule(BaseLightningModule):
    ...
    def forward_model(self, sample: dict[str, torch.Tensor], infer: bool) -> dict[str, torch.Tensor]:
        ...
        if infer:
            ...
            # SFC Breathness curve supervision
            predicted_variances = {}
            if 'breathiness' in predicted_variances and 'breathiness' in sample:
                losses['breathiness_mse'] = F.mse_loss(
                    predicted_variances['breathiness'], sample['breathiness']
                )
            if 'voicing' in predicted_variances and 'voicing' in sample:
                losses['voicing_mse'] = F.mse_loss(
                    predicted_variances['voicing'], sample['voicing']
                )
