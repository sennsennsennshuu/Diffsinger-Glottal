...
from lib.multiprocess import FailedItem
import tqdm
...
from lib.feature.glottal import compute_h1_h2, compute_hnr, compute_air_energy_ratio, compute_spectral_tilt
...
class BaseBinarizer(abc.ABC):
    ...
    def process_items(self, items: list[MetadataItem], prefix: str, augmentation=False, multiprocessing=True):
        ...
        with tqdm.tqdm(iterable, total=len(items), desc=f"Processing {prefix} items") as progress:
            for samples in progress:
                if isinstance(samples, FailedItem):
                    logging.warning(
                        f"Worker failed on a {prefix} item: {samples.exception}",
    ...
    @dask.delayed
    def extract_glottal_features(self, waveform: numpy.ndarray, f0: numpy.ndarray):
        """Extract SFC Breathness glottal features: H1-H2, HNR, air energy ratio, spectral tilt."""
        if not self.config.features.breathiness.enabled:
            return {}
        sr = self.config.features.audio_sample_rate
        h1_h2 = compute_h1_h2(
            waveform, f0,
            sample_rate=sr,
            frame_length=self.config.features.win_size / sr,
            hop_length=self.config.features.hop_size / sr
        )
        hnr = compute_hnr(
            waveform, f0,
            sample_rate=sr,
        ...
        air_ratio = compute_air_energy_ratio(waveform, sample_rate=sr)
        spectral_tilt = compute_spectral_tilt(
            waveform,
            sample_rate=sr,
            frame_length=self.config.features.win_size / sr,
            hop_length=self.config.features.hop_size / sr
