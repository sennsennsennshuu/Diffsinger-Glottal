...
class DiffSingerAcousticONNX(DiffSingerAcoustic):
    def __init__(self, vocab_size, out_dims, cross_lingual_token_idx=None,
        ...
        # Create aux_decoder for shallow diffusion (required by view_as_fs2_aux)
        shallow_args = hparams.get('shallow_diffusion_args', {})
        aux_decoder_arch = shallow_args.get('aux_decoder_arch', 'convnext')
        aux_decoder_kwargs = shallow_args.get('aux_decoder_args', {})
        if aux_decoder_arch == 'convnext':
            from modules.aux_decoder.convnext import ConvNeXtDecoder
            self.aux_decoder = ConvNeXtDecoder(
                hparams.get('hidden_size', 256), out_dims,
                num_channels=aux_decoder_kwargs.get('num_channels', 512),
                num_layers=aux_decoder_kwargs.get('num_layers', 6),
                kernel_size=aux_decoder_kwargs.get('kernel_size', 7),
                dropout_rate=aux_decoder_kwargs.get('dropout_rate', 0.1),
            )
        else:
            self.aux_decoder = None
        self.aux_decoder_grad = shallow_args.get('aux_decoder_grad', 0.1)

        # FusionAdapter: required for checkpoint compatibility (strict=True loading).
        # Not used during inference — ONNX forward path goes through forward_fs2_aux only.
        try:
            from modules.sweet_breathy.fusion_adapter import FusionAdapter
            self.fusion_adapter = FusionAdapter()
        except ImportError:
            self.fusion_adapter = None

        # SFC Breathness: glottal modules for ONNX export
        # The legacy __init__ path sets glottal_predictor=None; we recreate them here
        # if use_glottal=True and the import succeeds.
        if use_glottal:
            condition_dim = hparams.get('hidden_size', 256)
            try:
    ...
    def _compute_glottal_cond(self, cond: Tensor, f0: Tensor) -> Tensor | None:
        """Compute glottal condition residual for SFC Breathness.
        Returns None if glottal modules are not available."""
    ...
    def _apply_glottal_blend(self, cond: Tensor, f0: Tensor,
                              glottal_blend: Tensor | None) -> Tensor:
        """Apply glottal blend factor to condition.
        glottal_blend: [B, T] in [-1, 1], or None for no injection."""
        if glottal_blend is None:
            return cond
    ...
    def forward_fs2_aux(
        ...
        # SFC Breathness: apply glottal blend
        condition = self._apply_glottal_blend(condition, f0, glottal_blend)
        if self.use_shallow_diffusion:
            aux_mel_pred = self.aux_decoder(condition)
    ...
    def view_as_fs2_aux(self) -> nn.Module:
        ...
        del model.diffusion
        # Remove glottal modules — TransformerEncoder can't be exported to
        # ONNX opset 15 by PyTorch 1.13. Glottal injection is applied outside
        # the ONNX graph at inference time.
        model.glottal_predictor = None
        model.glottal_adapter = None
        if hasattr(model, '_glottal_proj'):
            model._glottal_proj = None
    ...
    def view_as_diffusion(self) -> nn.Module:
        model = copy.deepcopy(self)
        del model.fs2
        # Null glottal — TorchScript can't handle TransformerEncoder or -> dict
        model.glottal_predictor = None
        model.glottal_adapter = None
        if hasattr(model, '_glottal_proj'):
            model._glottal_proj = None
...
class DiffSingerVarianceONNX(DiffSingerVariance):
    def __init__(self, vocab_size, cross_lingual_token_idx=None):
        # DiffSingerVariance takes ModelConfig, not vocab_size.
        # Parse config YAML through full RootConfig pipeline for correct nested
        # deserialization (PredictionConfig, LinguisticEncoderConfig, etc.).
        from pathlib import Path as _P
        from lib.config.io import load_raw_config
        from lib.config.schema import RootConfig, ConfigurationScope
        root_dir = _P(__file__).resolve().parents[2]
        data_yaml = hparams.get('config_path')
        if not data_yaml:
            candidates = [
                root_dir / 'data' / 'config_variance.yaml',
                root_dir / 'configs' / 'variance_v3.yaml',
            ]
            for c in candidates:
                if c.exists():
                    data_yaml = str(c)
                    break
        if not data_yaml:
            raise FileNotFoundError('Cannot find variance config YAML')
        raw = load_raw_config(_P(data_yaml))
        cfg = RootConfig.model_validate(raw, scope=ConfigurationScope.VARIANCE)
        cfg.resolve(scope_mask=ConfigurationScope.VARIANCE)
        super().__init__(config=cfg.model)
        # Replace linguistic encoder / upsampler with ONNX wrapper.
        # Some DiffSingerVariance versions have self.fs2, others don't.
        if hasattr(self, 'fs2'):
            del self.fs2
        ...
        self.pitch_backbone_type = hparams.get('backbone_type', 'lynxnet')
        self.pitch_backbone_args = hparams.get('backbone_args', {})
        # v2 compatibility aliases for variance exporter
        self.use_melody_encoder = True
        self.variance_prediction_list = self.variance_list
        self.predict_dur = getattr(cfg.model.prediction, 'predict_dur', True)
        self.expose_expr = False
        from deployment.modules.fastspeech2 import LengthRegulator
        self.lr = LengthRegulator()  # ONNX view_as_* compat
        
        # v2-compat modules used by ONNX view_as_* forward methods
        H = hparams['hidden_size']
        self.pitch_retake_embed = nn.Embedding(2, H)
        self.delta_pitch_embed = nn.Linear(1, H)
        self.base_pitch_embed = nn.Linear(1, H)
        self.pitch_embed = nn.Linear(1, H)
        self.variance_embeds = nn.ModuleDict({
            v: nn.Linear(1, H) for v in self.variance_prediction_list
        })
        ...
        if self.predict_variances:
            if hasattr(self, 'variance_predictor'):
                del self.variance_predictor
            # Build v2-style ONNX args from v3 config (build_adaptor was removed)
            _cfg = cfg.model
            _var_ranges: list = []
            _var_clamps: list = []
            _norm = _cfg.normalization
            _pred = _cfg.prediction
            if _pred.predict_energy:
                _var_ranges.append((_norm.energy_db_min, _norm.energy_db_max))
                _var_clamps.append((_norm.energy_db_min, 0.))
            if _pred.predict_breathiness:
                _var_ranges.append((_norm.breathiness_db_min, _norm.breathiness_db_max))
                _var_clamps.append((_norm.breathiness_db_min, 0.))
            if _pred.predict_voicing:
                _var_ranges.append((_norm.voicing_db_min, _norm.voicing_db_max))
                _var_clamps.append((_norm.voicing_db_min, 0.))
            if _pred.predict_tension:
                _var_ranges.append((_norm.tension_logit_min, _norm.tension_logit_max))
                _var_clamps.append((_norm.tension_logit_min, _norm.tension_logit_max))
            _total_rb = _norm.variance_total_repeat_bins
            _bb_type = hparams.get('backbone_type', 'lynxnet')
            _bb_args = hparams.get('backbone_args', {})
            if self.diffusion_type == 'ddpm':
                self.variance_predictor = MultiVarianceDiffusionONNX(
                    ranges=_var_ranges, clamps=_var_clamps,
                    repeat_bins=_total_rb,
                    timesteps=hparams.get('timesteps', 1000),
                    k_step=hparams.get('K_step', 1000),
                    backbone_type=_bb_type, backbone_args=_bb_args,
                )
            elif self.diffusion_type == 'reflow':
                self.variance_predictor = MultiVarianceRectifiedFlowONNX(
                    ranges=_var_ranges, clamps=_var_clamps,
                    repeat_bins=_total_rb,
                    time_scale_factor=hparams.get('time_scale_factor', 1000),
                    backbone_type=_bb_type, backbone_args=_bb_args,
                )
            else:
    ...
    def forward_variance_reflow(self, variance_cond, steps=10):
        xs_pred = self.variance_predictor(variance_cond, steps=steps)
        return xs_pred

    def forward_variance_postprocess(self, xs_pred):
        # xs_pred: [B, num_vars, T]
        # Pass-through: per-variance clamping is handled by the engine.
        return tuple(xs_pred.unbind(dim=1))
