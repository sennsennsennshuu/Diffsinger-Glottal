...
from lib.vocabulary import PAD_INDEX
...
class FastSpeech2AcousticONNX(FastSpeech2Acoustic):
    def __init__(self, vocab_size, cross_lingual_token_idx=None):
        ...
        self.register_buffer(
            ...
            torch.LongTensor(cross_lingual_token_idx or []),
            persistent=False
        )  # [N,]
        ...
        # Create spk_embed if use_spk_id — needed for v3 key remapping
        # (v3 checkpoint: speaker_embedding → fs2.spk_embed)
        if hparams['use_spk_id'] and not hasattr(self, 'spk_embed'):
            self.spk_embed = Embedding(
                hparams.get('num_spk', 1), hparams['hidden_size'])
...
class FastSpeech2VarianceONNX(FastSpeech2Variance):
    def __init__(self, vocab_size, cross_lingual_token_idx=None):
        ...
        if len(cross_lingual_token_idx or []) == 0:
            self.use_lang_id = False
        self.lr = LengthRegulator()
    ...
    def forward_encoder_word(self, tokens, word_div, word_dur, languages=None):
        ...
        onset_embed = self.onset_embed(onset.float().unsqueeze(-1))
    ...
    def forward_dur_predictor(self, encoder_out, x_masks, ph_midi, spk_embed=None):
        midi_embed = self.midi_embed(ph_midi.float().unsqueeze(-1))
