...
class GaussianDiffusionONNX(GaussianDiffusion):
    ...
    def __init__(self, out_dims, num_feats, timesteps, k_step,
                 backbone_type, backbone_args, spec_min, spec_max):
        """Bridge v2 ONNX init → v3 dummy base.

        ddpm diffusion is not used in v3 (reflow path is standard), but
        supporting this init for deployment compatibility.
        """
        ...
        self.denoise_fn = cls(sample_dim, hparams['hidden_size'], **bb)
...
class PitchDiffusionONNX(GaussianDiffusionONNX, PitchDiffusion):
    def __init__(self, vmin: float, vmax: float,
        ...
        self.cmin = cmin
        self.cmax = cmax
        # Build backbone from v2-style args (same as GaussianDiffusionONNX)
        from utils.hparams import hparams
...
class MultiVarianceDiffusionONNX(GaussianDiffusionONNX, MultiVarianceDiffusion):
    def __init__(
        ...
        self.clamps = clamps
        # Build backbone from v2-style args (same pattern as GaussianDiffusionONNX)
        from utils.hparams import hparams
        bb_type = backbone_type or hparams.get('backbone_type', 'lynxnet')
        bb = {k.replace('dropout_rate', 'dropout'): v
              for k, v in (backbone_args or {}).items()}
        ...
        self.out_dims = repeat_bins
        self.num_feats = 1
        vmin_list = [r[0] for r in ranges]
        vmax_list = [r[1] for r in ranges]
        self.register_buffer('spec_min', torch.tensor(vmin_list, dtype=torch.float32))
        self.register_buffer('spec_max', torch.tensor(vmax_list, dtype=torch.float32))
        self.num_feats = 1
        self.out_dims = repeat_bins
